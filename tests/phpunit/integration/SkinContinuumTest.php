...
/**
 * Class ContinuumTemplateTest
 * @group Continuum
 * @group Skins
 */
class SkinContinuumTest extends MediaWikiIntegrationTestCase {
	...
	/**
	 * @return SkinContinuumLegacy
	 */
	private function provideContinuumTemplateObject() {
		$skinFactory = $this->getServiceContainer()->getSkinFactory();
		$template = $skinFactory->makeSkin( 'continuum' );
		return $template;
	}
	...
	public function testGetTemplateData() {
		...
		$title = Title::makeTitle( NS_MAIN, 'SkinContinuum' );
		...
		$continuumTemplate = $this->provideContinuumTemplateObject();
		$continuumTemplate->setContext( $context );
		...
		$openContinuumTemplate = TestingAccessWrapper::newFromObject( $continuumTemplate );

		$props = $openContinuumTemplate->getTemplateData()['data-portlets'];
		...
		$this->assertSame(
			[
				...
				'class' => 'mw-portlet mw-portlet-views emptyPortlet ' .
					'continuum-menu-tabs continuum-menu-tabs-legacy',
				'html-tooltip' => '',
				'html-items' => '',
				'html-after-portal' => '',
				'html-before-portal' => '',
		...
		$this->assertSame(
			'mw-portlet mw-portlet-namespaces continuum-menu-tabs continuum-menu-tabs-legacy',
			$namespaces['class']
		);
		$this->assertSame(
			'mw-portlet mw-portlet-variants continuum-menu-dropdown',
			$variants['class']
		);
		$this->assertSame(
			'mw-portlet mw-portlet-cactions continuum-menu-dropdown',
			$actions['class']
		);
		$this->assertSame(
			'mw-portlet mw-portlet-personal continuum-user-menu-legacy',
