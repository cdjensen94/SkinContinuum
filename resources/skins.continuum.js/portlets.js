...
function addDefaultPortlet( portlet, isDropdown ) {
	...
	ul.classList.add( 'continuum-menu-content-list' );
	const label = portlet.querySelector( 'label' );
	if ( label ) {
		const labelDiv = document.createElement( 'div' );
		labelDiv.classList.add( 'continuum-menu-heading' );
		if ( !isDropdown ) {
	...
	if ( wrapper ) {
		...
		wrapper.classList.add( 'continuum-menu-content' );
	} else {
	...
	portlet.classList.add( 'continuum-menu' );
	return portlet;
}

/**
 * A hook handler for util.addPortlet hook.
 * It creates a portlet based on the hint, and adabt it to continuum skin.
 *
...
function makeDropdown( content ) {
	...
	label.classList.add( 'continuum-dropdown-label' );
	label.setAttribute( 'aria-hidden', 'true' );
	const labelSpan = document.createElement( 'span' );
	labelSpan.textContent = label.textContent;
	label.textContent = '';
	labelSpan.classList.add( 'continuum-dropdown-label-text' );
	label.appendChild( labelSpan );
	const dropdown = document.createElement( 'div' );
	const checkbox = document.createElement( 'input' );
	const dropdownContent = document.createElement( 'div' );
	dropdownContent.classList.add( 'continuum-dropdown-content' );
	...
	checkbox.classList.add( 'continuum-dropdown-checkbox' );
	checkbox.setAttribute( 'aria-label', label.textContent || '' );
	dropdown.id = `${ id }-dropdown`;
	dropdown.classList.add( 'continuum-dropdown', `${ id }-dropdown` );
...
/**
 * A hook handler for util.addPortlet hook.
 * It creates a portlet based on the hint, and adapt it to continuum skin.
...
function addPortletHandler( portlet, before ) {
	...
	if ( isDropdown ) {
		const pageToolsDropdown = document.querySelector( '#continuum-page-tools-dropdown' );
